
# from adventofcode.inputs import get_input
# from adventofcode.utils import aoc_timer
import logging
import math
from heapq import heappop, heappush
from collections import Counter
from math import radians

logging.basicConfig()
import numpy as np
import math

logger = logging.getLogger(__name__)

N = -1j
S = 1j
E = 1
W = -1

import re

def rotation_x_3d(vec, degrees):
    rad = math.radians(degrees)
    rot = np.array([[ 1, 0 ,0],
                     [ 0, math.cos(rad) ,-math.sin(rad)],
                     [ 0, math.sin(rad) ,math.cos(rad)]])
    return vec @ rot

def rotation_y_3d(vec, degrees):
    rad = math.radians(degrees)
    rot = np.array([[ math.cos(rad), 0 ,math.sin(rad)],
                     [ 0,  1, 0],
                     [ -math.sin(rad), 0 ,math.cos(rad)]])
    return vec @ rot

def rotation_z_3d(vec, degrees):
    rad = math.radians(degrees)
    rot = np.array([[math.cos(rad) ,-math.sin(rad), 0],
                     [ math.sin(rad) ,math.cos(rad), 0],
                     [0, 0, 1],
                     ])
    return vec @ rot

class CubeFace(object):
    def __init__(self, orig_coord, size, text, a_xyz):
        self.orig_coord = orig_coord
        self.size = size
        self.text = text
        self.a_xyz = a_xyz
        # self.inv_cube_coords = {tuple(v):k for k, v in cube_coords.items()}
        # self.neighbours = {N: None, S: None, W: None, E: None}

    # def add_neighbour(self, dir, cube):
    #     self.neighbours[dir] = cube

    def __repr__(self):
        return f"Face[A={tuple(self.a_xyz)}]"


def rotate(xyz, cube_size, d):
    if d == S:
        #  south rotation is r_x + 90
        new_pos = np.rint(rotation_x_3d(xyz -np.array([1,1,1])* cube_size/2, 90)+np.array([1,1,1])* cube_size/2).astype(int)
    elif d == N:
        new_pos = np.rint(rotation_x_3d(xyz-np.array([1,1,1])* cube_size/2, -90)+np.array([1,1,1])* cube_size/2).astype(int)
    elif d == W:
        new_pos = np.rint(rotation_y_3d(xyz-np.array([1,1,1])* cube_size/2, -90)+np.array([1,1,1])* cube_size/2).astype(int)
    elif d == E:
        new_pos = np.rint(rotation_y_3d(xyz-np.array([1,1,1])* cube_size/2, 90)+np.array([1,1,1])* cube_size/2).astype(int)
    else:
        raise RuntimeError(f"guard {d}")
    return new_pos

def parse_input(i):
    maze_str, instructions_str = i.split("\n\n")
    maze = {}
    entrance= None

    for y, row_str in enumerate(maze_str.split("\n")):
        for x, c in enumerate(row_str):
            maze[x + y*1j] = c

        if y == 0:
            x_entrance = row_str.find(".")
            entrance = x_entrance + y * 1j

    dim = max(len(row_str) for row_str in maze_str.split("\n")), len(maze_str.split("\n"))
    instructions = [ inst.group() for inst in re.finditer("([0-9]+|[LR])", instructions_str)]

    if entrance is  None or     dim is None:
        raise RuntimeError("GUARD")

    cube_size = max(dim) // 4

    maze_str_lst = maze_str.split("\n")
    x_first_face = min(maze_str_lst[0].find("."), maze_str_lst[0].find("#"))

    a_xyz = np.array([0, 0, 0])

    # cube_coords = {
    #     'A': np.array([-1, 1, 1]) * cube_size / 2,
    #     'B': np.array([1, 1, 1]) * cube_size / 2,
    #     'C': np.array([-1, -1, 1])* cube_size / 2,
    #     'D': np.array([1, -1, 1])* cube_size / 2,
    #     'E': np.array([-1, 1, -1])* cube_size / 2,
    #     'F': np.array([1, 1, -1])* cube_size / 2,
    #     'G': np.array([-1, -1, -1])* cube_size / 2,
    #     'H': np.array([1, -1, -1])* cube_size / 2,
    # }

    all_cubes = [
        CubeFace(
            x_first_face + 0j,
            cube_size,
            [l[x_first_face: x_first_face + cube_size] for l in maze_str_lst[0:cube_size]],
            a_xyz,
        )]

    while len(all_cubes) < 6:
        for c in all_cubes:
            for d in [N, S, W, E]:
                next_corner = c.orig_coord + d*cube_size
                if 0 <= next_corner.real < dim[0] and 0 <= next_corner.imag < dim[1]:
                    if maze.get(next_corner, " ") != " ":
                        existing_cube = [c for c in all_cubes if c.orig_coord == next_corner]
                        if len(existing_cube) == 0:
                            new_a_xyz = rotate(c.a_xyz, cube_size, d)
                            new_cube = CubeFace(
                                next_corner,
                                cube_size,
                                [l[int(next_corner.real): int(next_corner.real) + cube_size] for l in maze_str_lst[int(next_corner.imag):int(next_corner.imag)+cube_size]],
                                new_a_xyz,
                            )
                            all_cubes.append(new_cube)
    cube_dict = {}
    for c in all_cubes:
        cube_dict[tuple(c.a_xyz)] = c
        cube_dict[c.orig_coord] = c
    if len(cube_dict) != 12:
        raise RuntimeError(f"guard sides cubes_dict != 6 {len(cube_dict)}")

    return entrance, maze, dim, instructions, cube_dict

def exec_inst(pos, dir, inst, maze, dim, cube_dict, part2=False):
    '''
    execute 1 instruction
    :return:
    '''
    if inst == "R":
        return pos, dir*1j
    elif inst == "L":
        return pos, dir*-1j
    else:
        cube_size = max(dim) // 4
        steps = int(inst)
        _orig_pos = pos
        _orig_dir = dir
        while steps > 0:
            next_pos = pos + dir

            while (c:= maze.get(next_pos, " ")) == " ":
                if part2:
                    curr_top_left_2d = int(pos.real - (pos.real % cube_size)) + int(pos.imag - (pos.imag % cube_size)) * 1j
                    curr_cube = cube_dict[curr_top_left_2d]
                    next_a_xyz = rotate(curr_cube.a_xyz, cube_size, dir)
                    raise RuntimeError("WTF")
                else:
                    next_pos += dir
                    next_pos = (next_pos.real % (dim[0]+1)) + (next_pos.imag % (dim[1]+1)) * 1j

            if c == ".":
                steps -= 1
            elif c == "#":
                return pos, dir

            pos = next_pos
        return pos, dir

def solve_puzzle(entrance, maze, dim, instructions, cube_dict, part2=False):
    turn = 0
    dir = E
    pos = entrance

    for inst in instructions:
        new_pos, new_dir = exec_inst(pos, dir, inst, maze, dim, cube_dict, part2)
        turn += 1
        if logger.level <= logging.DEBUG:
            logger.debug(f"Turn {turn} == {pos} | {dir} -- {inst} -> {new_pos} | {new_dir}")
        pos = new_pos
        dir = new_dir

    return 1000 * (pos.imag + 1) + 4* (pos.real+1) + {E: 0, S:1, W: 2, N: 3}[dir]




def print_state(elves_coord_lst, turn):
    '''
    Debug function to print the grid
    '''
    pass
    # logger.debug(f"Turn{turn}\n" + "\n".join(row_lst))



if __name__ == '__main__':
    # logger.setLevel(logging.DEBUG)
    logger.setLevel(logging.INFO)

#     example = '''.....
# ..##.
# ..#..
# .....
# ..##.
# .....'''
#     elves_coord_lst = parse_input(example)
#     t = solve_puzzle(elves_coord_lst)
#     print(t)

    example = '''        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5'''


    entrance, maze, dim, instructions, cube_dict = parse_input(example)
    t = solve_puzzle(entrance, maze, dim, instructions, cube_dict, part2=True)
    print(f"example 1 solution: {t}")
#
#     part1 = '''                                                  ......#....#.#........................................#.......##...........#.............#......#...
#                                                   ..............#..#........#......##........................#......#....#........................#...
#                                                   .....................#.........#...#.#........#..............................#....#........#..#.....
#                                                   .........#.........##................................#.....#.........#...........#...........#......
#                                                   ..........#.............#.....##.......#..#..........##......#.#...........#............##..........
#                                                   ......#.............##.........#.....#....#...............................................#.......#.
#                                                   ..#......#......#.........#..#.........#....#...................#...........#...#.....##.#..#...#..#
#                                                   #.#.............#.............#.#......#.....#.....#.##.##.....#...#..#...............#.............
#                                                   .#....#...##....#...................#..#............................................#.........#.....
#                                                   ....#..##....#..#....#..............#..#...#...#................#...........#..#..#............##...
#                                                   ..#.#....................................................................#.....##....##.....#..#....
#                                                   .......#...#......#...##........#...#......................#.#.#..............#...#..#.....#........
#                                                   .....#.#..#........#..............##......#........##...............#..................#.....#......
#                                                   .......#...............#...#...................................#............#.......................
#                                                   ......#..##.....#.......#....#..................................................#.#.......#.........
#                                                   ................#..#..#....#.#.....................#................................................
#                                                   .................#.#........#.....##...#...#.#..#....#....#.............#...#..................#..#.
#                                                   .#...................#......#..#...............#.............#.........................#...#.....#..
#                                                   ...#.#...................#...#.........................#.......##.......................#.........#.
#                                                   .#.............................#........##.........#.#.....##.....................#....#...........#
#                                                   ..........#.....#..............................#................##.....................#.......#....
#                                                   .....................................#....##..........#...............#.......##.........#.#........
#                                                   ....#.###............#....#..........#..........#.......................#...............#....#.#....
#                                                   .#...#...#...............#.........#........................#.................#..................#.#
#                                                   .................#......#.....................#........#......#.....................#....#.##.......
#                                                   ##..#....................#.....................#....#....#.....#.............................#.....#
#                                                   #...#.......................................................##.#...#.........#..#...................
#                                                   ............#......#.....#........#..#.#......#.#.............#....#......#.#...............#.......
#                                                   ..................................................#.#..........##.........##......#.#.#.........##..
#                                                   ..#..........................#.......#..#..#.......#.................###.......#...........#..#...##
#                                                   ..#.....#..#...#.....#.....#...#..#.................................#.....#.........#.............#.
#                                                   ............#.#..#..##..#.#.##....#...............#.......#........#............#........#..........
#                                                   .#...........#...........................#............#.....#.#.........#.....#.......###...#.......
#                                                   ...#.......................#.........................#...##........#.....................#.......#..
#                                                   ...........#..#.............................#.................#..##...#.............................
#                                                   ..................#.#...#..#....#....#..........#................#..#.........#......#.......#......
#                                                   ...#...#....#.#................#.............#.............................#..#.....................
#                                                   ....................#.........#..#.#...#.#.##...#...#..............#.......#.........#.............#
#                                                   ...#.....#.............##.....#....#.#.......#......................................................
#                                                   ..........#..#...#.......................#.................##.....#........#...#......#.......#.....
#                                                   ........#......#...............#.#.....#...........................#.....................#.......#..
#                                                   ..................................#...........#......#...........#.............#....###.............
#                                                   ................#.......................#.#......#.##..........#......#............#................
#                                                   .........................................#..........................................##...........#..
#                                                   ...#.......#....##.#....#...............#..#...#.............#...#....#.....#.#...........#.........
#                                                   #........#...........#...........#.....................#........#..................#.....#......#...
#                                                   ........#.....##..#.........................#....#..........#.##..#.......#.#.................#...#.
#                                                   ..........................#.............................#........#............#.............#....#..
#                                                   .............................#..................................................#...................
#                                                   ................#.............#..#..........##......#......#.........#...........##............##...
#                                                   #...#.......#.............#............#..........
#                                                   #................#...#....#..................#....
#                                                   .................#.....#.#....#......#...#........
#                                                   ..................................#...............
#                                                   .....#...............................#............
#                                                   ...............#............##..........#.......#.
#                                                   #............#.....................###.#.........#
#                                                   .#...............#...........#....#......#........
#                                                   .#.............###.#................#........#....
#                                                   ....##............#..........#.#.........#.#.#....
#                                                   ..................#..#..............#......#......
#                                                   ....#.............................#...............
#                                                   .#.#..........#...........#......##...............
#                                                   ..........#......#....#...#.#....#..#.............
#                                                   ................#...............#.................
#                                                   ..........#..#...#..............#...............#.
#                                                   ...#..............##..............#...............
#                                                   ..#...#.#....#.........#.........#.......#........
#                                                   .................#..............#.........#.....#.
#                                                   .......#........#......#............#......#......
#                                                   ..........#..............#....#..#............#...
#                                                   .......#............#.............#...............
#                                                   .....#.#..........#.#..........#..................
#                                                   #...#...................#......#......#.....#.....
#                                                   ........#........##.#.......##................#...
#                                                   ..................#.........................##....
#                                                   ..#................#..............#.#....#.#.#...#
#                                                   ........#..........................#..........#...
#                                                   ..#.......#.#............#..........#.....#.......
#                                                   ....#....#...............................#........
#                                                   ..#............................................#..
#                                                   ....#.......#..............##.......#......##.....
#                                                   .#..........#......#....#.#........#..............
#                                                   ................#.#.......##.............###.....#
#                                                   ...............#......##.#...#................#...
#                                                   ....#.......#....##...#..#..........#.........#...
#                                                   .....#.........#...#...........#........#.........
#                                                   .......#...............#..................#.......
#                                                   ...........#.....#.........#........#....#.......#
#                                                   ..#.....................................#...#.....
#                                                   ..................#..............#....#.#.........
#                                                   ........................#.......#.#......#........
#                                                   ...#......#..#..........#..#..#........#..........
#                                                   .....#.......#.........#.............#.......#....
#                                                   ......#......#...............#.....#.........#....
#                                                   .#.........................#.....#.............#..
#                                                   ...#......#.#.....##...#.#....#.#...............#.
#                                                   .............#........#.#.#.....#.................
#                                                   ...#.........#.........#....#.................##..
#                                                   .......#.......##...#....#..........#.#...........
# ..#......#....................#.........#........#.....................#............#.....#.........
# .##...........##...................#.#..................................#...........................
# ....#............#..........#..........................#...#......................#....##...........
# .#...........#........................#..............................#..#...................#......#
# #................#.#........#...........................#....................#...........#..........
# #.....................#.....#.......##....#......#..##...#...........#...................#..........
# .........#..#......#....#......#....#.......#.....................................................##
# ..#...#......##.#..................#.......#.............#....#....#.........#..............#.....#.
# ...........#...#...#.#........#..................##...........#.....#...............................
# .............................#....##..#..#..........#.............................#.................
# ...#.................#...#...#...#.................................................#..............##
# ......#........#........#.........##....#....#.....#.......#.............#...##.##.#................
# ..#.......................#....................#..#...#.................#......#..........#......#..
# .........................#....#..##..............#.............#.#...#....#.......#...#....#.....#..
# ....#................#.#...........................#....#..#.......##.#.....#................#.#....
# ..#.......................#.....#...#..#..#...#...#.............................#............#......
# #..............................................#.......................................##........#.#
# ..............#.#............##..........#.........................#..................#.#...........
# .....#......................#...........#.....#.................#................#........#...#..#.#
# ......#.#....#......#.#..................#.#................#.#.#...........#............#.....#....
# .#......#......##..............#.........#.#.#.#..................#...#..........#..................
# ...#...........#.............#....#.#...#....#.......................#.#.....#.....#..............#.
# ...#...#.#.........#...#............#....#.........#.....#......#..#...........#.....#......#.......
# .#.....#..........#..............#..#........#..............#...#.............#.#.......#..#........
# .....#........................#.....#......#...............##..#.#...#...#.#............#...........
# .....#......#.........#....#.......##.......#.##..................#...............#......#........#.
# .#..#..#..#.................................#......#.#............#..#....#...................#.....
# .#..#...........................#....#............................................................#.
# ......#..............................#..#.......#.........##..........##...#........................
# ............#...............#..#..#..........#......................#...#.#............#............
# ...##..................#.....#.........#......#...........#.......#.....#.......................#...
# ..#..#..#......#......#.....#.......#.................##..#.........................................
# ............#...##...#..........#.................................##....#........##........#........
# ...##...#....#....#........................#.............##.........................##.......#......
# .............##..................#...................##.......#........##.............#.#..........#
# .......#.....#...##..............#..........................#..........#..........#..#.............#
# ............#...#........#........#...............#.....#.....#.......#..............#.....#......#.
# ...........#.....#..##.................#......#........#.#.....................#....................
# .....#......#.........#..........#..#......................................................##.......
# ..........#.....#......#.#.........#.....#..................##.............................#........
# ##......#...#..........#...............#.............#........#..................#........#........#
# ..#.........#...........#.......#...........##...................#.................#.#.....#...#....
# ...#.........#..................................#.....#..#............#.............#.#.............
# ....#...........#..........#..............#..###........#..#.........#....................#.#.......
# ..#..........#..#...............................#...#.................#............#............#.#.
# .....................#.............#....#..#....#......................#..#......#..................
# ........#.#.............#....#...#.#.....#..........#................#.......#........#....#.#.....#
# ##.........................##....#...............#..........#......................##...............
# .........#..................................#........................#....#............#...#.#......
# ........#........................#..#...#..........................#................................
# ................#..#.......#..#...................
# .............#........#...................#.......
# ..#....#.......................#..........#...#..#
# ..........#.....................#..............#.#
# .........#...#...#..............#..#.........#....
# ...........#.....#................................
# .......#..#..#.......#............#...............
# .........................#..........#..#....#..#..
# #...........#....#.....#....#.#..#.......#........
# #.#.......#.....#........#....#...#...............
# ..............................................#...
# #......#....................................#...#.
# ........##..#.....#.#......#.........#..##....#...
# ...............#.........#...#.......#......#..#..
# ..........#.#......#.....#........................
# .....#.....#......................................
# ..#..#............#.......#.....#..#.......#...#..
# ...............##....#........#..#.............#.#
# ..............#..................#.....#......#...
# #.......#.......#...#...............#.#....#......
# #.........#........#..........#........#....#.....
# .....###.#...........#...........##............#..
# .........#..##.......#......................#.....
# .....#.##......#.......#...#.............#........
# ##.................#.....#.............#....#...#.
# ........#..............#.#.........#.......#......
# ........#..#.#.....#.............#............#..#
# .#.....#.#..#..#.##........#..........#.....##....
# .....#....#.........#.............................
# ................#..#....#.....#....#........#..#..
# #.#............#.....#.............#.........#....
# ...#....#..........#.#.................#.#..#...#.
# ........#................#...#......#......#...#.#
# ..#.............#....#.......#..............#.....
# .....................................##.........#.
# ............#.#...................................
# ..#...........................#..........#........
# ...#.............#..............#..........#...#..
# #......#...#...#...#...#..................#....#..
# ....#......#..#....#.#.#..........................
# ..#.......................#..............#..#.....
# ................#........#...#..........#.#.......
# ....#...#......#...................#.#.#...#....#.
# ............#..............#........#..#.....#....
# ..........#...............#.......#...............
# #....................#.......#...........#........
# .......#.........#..........#.#..#.#..............
# ..#.........##......................#...#.........
# ...#.................#.##............#........#...
# ....#......#..#.....................#.............
#
# '''
#
#     # entrance, maze, dim, instructions = parse_input(part1)
#     # t = solve_puzzle(entrance, maze, dim, instructions)
#     # print(f"part 1 solution: {t}")
#
#     entrance, maze, dim, instructions = parse_input(part1)
#     t = solve_puzzle(entrance, maze, dim, instructions, part2 = True)
#     print(f"part 2 solution: {t}")
